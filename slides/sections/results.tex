...
\begin{frame}
	\frametitle{\secname: groups}	
	For the analysis the users were divided into groups using Body Mass Index (\textit{BMI}).\\~\

	% dico che sono divisi in base al BMI, quindi anche la soglia --> questionario peso e altezza
	% dico che ogni utente è stato classificato come rp 
	\begin{center}
		\includegraphics[scale=0.17]{users_by_group.png}\\~\
	
		\begin{table}[]
			\begin{tabular}{c||c||c}
				\textbf{Abbreviation} & \textbf{Extended form} & \textbf{Criteria}\\
				\hline \hline
				\textbf{LBU} & \textbf{L}ow \textbf{B}MI \textbf{U}sers & $ BMI \leq 21,5 $\\
				\textbf{HBU} & \textbf{H}igh \textbf{B}MI \textbf{U}sers & $ BMI > 21,5 $\\
				\hline
				\textbf{RP}  & \textbf{R}egular \textbf{P}layers & frequently \\
				\textbf{CP}  & \textbf{C}asual \textbf{P}layers & rarely
			\end{tabular}
		\end{table}
...
\begin{frame}
	\frametitle{\secname: research questions}
...
\begin{frame}
	\frametitle{\secname: analysis of physical characteristics}
...
\begin{frame}
	...
	Both personal opinions and scientific data confirm us that there is a correlation between \kt{} teaching and physical characteristics.
\end{frame}

\begin{frame}
	\frametitle{\secname: analysis of confidence with the pad}
...
\begin{frame}
	\frametitle{\secname: analysis of confidence with the pad}
	This figure shows the precision of the waypoints of the task $ t1 $ taken by the two groups.
	% waypoints e ratio
	
	\begin{figure}
		\centering
		\begin{tikzpicture}
		\node(a) {\includegraphics[scale=0.15]{waypoints_tt_t1.png}};
		\node[] at (4, 0) {LBU};
		\node[] at (4, -0.5) {HBU};
...
\begin{frame}
	...
	The mental efforts in \te{} teaching are: $ 5,6 $ for \smallusers{} and $ 6 $ for \bigusers{}. Also, the data confirm us how users confident with the pad are better in \te{}.\\~\
	
	Furthermore, from the figure relative to times it's possible to notice how \te{} is always slower than \kt{}.
\end{frame}