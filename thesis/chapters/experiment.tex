...
In this chapter how the experiment was prepared and the materials used for the tests are explained. In addition, a complete and detailed description of the experiment is provided in section \ref{experimental-flow}, starting with the experimental design that was divided into three phases and described in every step. Later the data collected are explained. Finally a overview about the participants (who they are and their peculiarity) is provided. 
...
In figure \ref{fig:shapes}, there are three objects of different shape created using wood. The figures are about $ 8x8$ $ cm $ in width and length and about $ 2 $ $ cm $ high. Above them has been placed another little cube of $ 3x3x2 $ $ cm $ for proper grasping (fig. \ref{fig:task_frontal}). With the gripper available is not possible to use smaller objects without creating special supports to facilitate grasping. After that, some tests were conducted with these objects before submitting the final experiment to the users. 
The task was considered as a realistic assembly condition, where a pick and place task with large margins (figure \ref{fig:simple_interlocking}) was used as a simple task, while the task in figure \ref{fig:difficult_interlocking} was used to simulate a difficult assembly task with interlocking.
The entire setup is shown in figure \ref{fig:setup} and a complete description of the experiment is provided in section \ref{experimental-flow}.\\

\begin{figure}[H]
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[scale=0.05]{shapes.jpg}
	...
	\end{subfigure}
	\caption{Shapes used for the tasks}
	\label{fig:entire_shapes}
\end{figure}

\begin{figure}[H]
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[scale=0.045]{task2.jpg}
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[scale=0.045]{task3.jpg}
		\caption{Difficult interlocking}
		\label{fig:difficult_interlocking}
	\end{subfigure}
	\caption{The first and second task}
...
This section describes the entire flow which is done by every participants of the experiment. It has been divided into three stages for convenience: the first is the phase before the experiment, the second is the experimental phase and the last one is the final phase after the experiment.


\subsection{Pre-experiment phase}
\label{pre-experiment-phase}
In this phase all the necessary procedures to correctly conduct the experiment are explained. First of all a privacy release form for data collection was signed by the users. After that, a teach phase was carried out by all the users in order to understand how they can move the robot (in \kt{} or \te) and which buttons they need to press to perform the actions. At this stage users were asked to move a Lego block from a pick position to a place one performing a $ 90 $° rotation to teach him how to use rotation feature. This simple pick and place was performed by the users in both the modalities. When the user completed this phase an ascending unique id was assigned to him/her. Moreover, every user performs the pre-experiment and the experimental phase without seeing other users' trials.

\subsection{Experimental phase}
\label{experiment-phase}

From the two modalities proposed in \ref{teleoperation-pad}, the \te{} modality chosen for the executions of the task was the \textit{full task space}. It allows the correct execution of the task in a simple way. The experiment consists in two tasks in ascending order of difficulty, repeated for \textit{three} times both for \kt{} and \te{}, where the shapes shown in section \ref{fig:shapes} must be entered as in figure \ref{fig:simple_interlocking} and figure \ref{fig:difficult_interlocking}. We call the repetitions \textit{``trial''}. After every trial a \textit{vote} to the work done (based on time, number of waypoints and action on gripper) is given to the user. The formula for calculating the vote was:
\begin{center}
	$ (sample*0,1/60) * (opt\_wp/\#wp) * (opt\_act/\#act)$
\end{center}
where the first term of the formula calculates the time in seconds, the second term calculates the ratio between the number of optimal waypoints and the number of waypoints taken by the user. Finally, the third term calculates the ration between the number of optimal actions on gripper and the actions taken by the user. A generic vote is given to the user instead of reporting the time to complete the experiment. This evaluation metric was chosen because, if the times is given, the user will try to perform the next trials in less time because it competes with itself and he/she risks to complete the tasks incorrectly. All votes that have been given after each trial were noted. Moreover, some comments about the execution of the tasks have been written and reported in the next sections.
...
\begin{itemize}
	...
	\item if there is an error that does not depend by the user, the trial is repeated,
...
At the end of the experimental phase the users were asked to fill out the questionnaire available in \cite{questionnaire}. This questionnaire is divided in three parts: the first part asks personal informations as name, age, approximate height and weight and other questions to better understand how to divide users for the result analysis. The second part resumes some questions from the questionnaire of \textit{Nasa TLX} described in \cite{nasa-tlx}, and the last part asks general questions or optional comments about the experiment. 

\section{Data collection}
\label{data-collection}

During the experiment some data were collected from the ROS\texttt{ topic} available with sample rate of $ 10 Hz $. All these data have been saved in \texttt{.csv} files. These files are related to: 
\begin{itemize}
	\item \texttt{user\_x.csv} : sequence of actions performed by the users during the experiment,
	\item \texttt{user\_x\_wrench.csv} : force applied on EE,
	\item \texttt{user\_x\_pose.csv} : position and orientation of the EE,
	\item \texttt{user\_x\_joint\_pos\_vel.csv} : position and velocity of all joints,
	\item \texttt{user\_x\_joint\_ext\_tor.csv} : torque and external torque of all joints. 
\end{itemize}
...
In questionnaire, data on physical characteristics as weight and height were collected to see if there were correlations between the use of \kt{} teaching and the physical characteristics of each user. In figures \ref{fig:weight} and \ref{fig:height} are represented the percentages of users divided by weight and height, grouped into intervals of $ 10kg $ for weight and of $ 0,1 m $ for height. \\
