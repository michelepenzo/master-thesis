...
\begin{itemize}
	...
	Finally both the values are lower than those of the mental effort in \te{} makes us understand that \kt{} is easier to use as it moves the robot manually without having to remember how the analogs must be used to move the robot. Instead, in \kt{} only one button that has two codes (open/close gripper and save waypoints) is used unlike the \te{} where there are more than one button.
	
	\item \textbf{Physical effort in \te{}}
	
	The concept of physical effort in \te{} doesn't make much sense as there isn't interaction with the robot. In fact, the values obtained from the two groups are identical: $ 1,2 $ and these values are very low. In figure \ref{fig:wrench_tt} it's possible to immediately spot that the force applied on EE during the experiment are close to zero, therefore null. There are only a little force due to the fact that the EE is not completely balanced. In fact, \te{} puts users at ease because it minimizes the interaction forces between the robot and the user. It's possible to say that \te{} unifies the differences between the physical characteristics.
...
\noindent
From the data collected and analyzed it's possible to resume the pros and cons of both the modalities:
\begin{itemize}
