...
\noindent
In figure \ref{fig:time_and_distance_group_t2} we can see two figures which indicate the same values of previous tables. The figure \ref{fig:time_groups_t2} represents graphically the table \ref{tab:time} and the values on $ x $ axis are in seconds. Instead in figure \ref{fig:distance_groups_t2.png} there are the data represented in table \ref{tab:distance}. The values on $ y $ axis are in meters. From these figures is possible to quickly understand the differences between the two groups and the different modalities. It's possible to understand the correlation between time and distance. Only times and distances for the task $ t1 $ are represented in the these figures: the figures for task $ t2 $ are similar but with the values described in the relevant table.
...
\noindent
In figure \ref{fig:wrench_all} there is another representation of \texttt{cartesian\_wrench} calculate on $ x,y,z $. This figure represent the difference between \kt{} and \te{} teaching between the two groups always divided by physical characteristics. All the boxplots shown in both the figures are divided in three groups (a group for each value of the wrench $\{ x,y,z \}$) and every group has two boxplots: the first one for \smallusers{} group and the second one for \bigusers{} group.
