...
This section describes the differences between our work and the main works that have been considered in the previous sections. 
From \cite{robot-learning}, the authors compare the same interactions modalities used in our work but the \te{} modality was implemented with an haptic device. This work highlight how \kt{} and the \te{} from them developed does not allow to take full advantage of the collaborative functionalities. Moreover, their haptic device is not intuitive to use: in fact, our \te{} method, was developed using a common joypad for video game.
From \cite{comparing-teleoperation}, the scheme of the \te{} control that was developed, was described in section \ref{teleoperation-pad}. Our user test, similar to the work done in \cite{user-study-ks}, consider a similar assembly tasks but they use a methodology for the evaluation only based on the force applied on a balance. In our user experiment, there were multiple evaluation metrics as described in chapter \ref{results-analysis}.
