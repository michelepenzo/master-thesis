...
In figure \ref{fig:diff_kt} and \ref{fig:diff_teleop} are represented on $ x $ axis the five repetitions of the users, while on $ y $ axis the time in seconds necessary to complete each repetition. As represented in figure \ref{fig:diff_kt}, it is possible to say that the time necessary to complete the task in \kt{} teaching was similar between the two users, but in figure \ref{fig:kt_user} the first trial was not completed due a mistake by the user. In this case, the experiment was not repeat because this was a test and all the possible problems had to be understood. We can also say that with more attempts the time for completing the task decrease.
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}%		
		...
		\label{fig:kt_me}	
	\end{subfigure}
	\caption{Difference between users in \textit{KT}}
	\label{fig:diff_kt}
\end{figure}

In figure \ref{fig:diff_teleop} we can say that the times to complete the task in \te{} are higher with respect to \kt{}. We can also observe that the inexperienced user was always slower than the user with experience. Even in this case the inexperienced user made a mistake (figure \ref{fig:teleop_user}) because he made a collision with a Lego block, but this repetition was included anyway in the results.
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\label{fig:teleop_me}
	\end{subfigure}
	\caption{Difference between users in \textit{TT}}
	\label{fig:diff_teleop}
\end{figure}

From the two failures, we noticed that the user did not know very well the environment and how to use the robot. 
Therefore, in the final experiment, a thorough explanations about \kt{} and \te{} was provided to the user before starting the experiment, but the failed trials were still considered. Moreover, from this experiment, it is noticed that three trials are enough to see the learning results.
