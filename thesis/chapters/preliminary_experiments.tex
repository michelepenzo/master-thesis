...
This chapter describes how the experiment was prepared and the materials used for the tests.
 
%----------------------------------------------------------------------------------------

\section{Preliminary hardware setup}
\label{preliminary-hw-sw}
The first thing that was done was the calibration of the gripper using the application provided by Kuka and usable from the SmartPad. This application consists on moving the sixth and seventh joint with multiple predefined movements around each selected axis. This calibration provides a way to calculate the weight, center of mass and inertia matrix of the tool attached on media flange. This operations were repeated in different configurations to provide more robust estimations. The chosen configurations were the pick and place position, therefore the most used during the experiment. Finally, the configured tool was loaded as \texttt{ros\_param} and recognized from the stack.
...
The topic called \texttt{CartesianWrench} shows the external wrench measured by the robot according to the used reference frame. The wrench is composed of a vector that represents the force along the three spatial dimension: from this vector only the force along $ z $ was extracted. In our case, the force is measured on the gripper. To understand if we could consider the force applied on the EE, some experiments were conducted to understand the reliability and responsiveness of the values returned from this topic. These tests were conducted using objects of different weight calibrated on a scale with precision to the gram and attached to both of the two jaws of the gripper. Every weighing was performed in three different configuration of the robot and for two times. The pose $ p1 $, or home pose, is with the elbow high in start position, the pose $ p2 $ is with the gripper perpendicular to the pick position used in the experiments and the pose $ p3 $ is the place position used in the experiment. In table \ref{tab:wrench} the columns indicate the values, expressed in $ N $, of the force calculated and extracted from the cartesian wrench for each weights. The penultimate and the last rows indicate the mean and the standard deviations, while the third last row indicates the the expected value: $ weight*9.81 $.
% todo foto con le 3 configurazioni
\begin{table}[H]
	\centering
...
Preliminary experimental was made by an inexperienced user who had to do the same task for $ N $ times.The value of $ N $ that was chosen is $ 5 $. This relatively high value that was reduced in the final experiment, was chosen precisely to have as much data as possible to analyze after this preliminary experiment. It was also chosen to understand if the user could improve with multiple repetitions of tasks.
The task, in figure \ref{fig:pre-task}, consisted to move four Lego blocks from a pick position to a place position. This task was repeated for both the modalities in section \ref{project-implementation}.
% todo cerchiare con tikz
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.15]{pre_task.jpg}
	\caption{Preliminary experiments trials with Lego blocks}
	\label{fig:pre-task}
\end{figure}

The data collected provides a solid basis to perform a preliminary analysis and understand how to perform the final experiment by collecting only the data for the final evaluation.
...
From the two failures, it's noticed that the user doesn't know very well the environment and how to use the robot. Before the final experiment a thorough explanations about \kt{} and \te{} is provided to the user and the failed trials are still counted. Moreover, from this experiment, it's noticed that three trials are enough to see the learning results.
