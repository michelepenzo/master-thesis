...
As described in \cite{impedance}, impedance control imposes a desired dynamic behavior to the interaction between the robot EE and environment. The desired performance is specified through an set of equation of mass-spring-damper, each one for all the directions of the cartesian space. A model that describes how the reaction forces are generated in association with the environment deformation is not explicitly required, in fact there is no way to regulate the contact force with position control.
