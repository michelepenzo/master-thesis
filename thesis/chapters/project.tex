...
This chapter describes the general setup and its components and present a small overview about tools used to develop the project. Finally, the project is explained.
...
\Te{}, or \textit{remote control}, indicates the movement of the robot at a distance. In our case, \te{} is intended as control of the \kuka{} with a \textbf{PlayStation 4 pad} and it was developed in two ways:
...
\begin{itemize}
	...
	\item \textit{reduced joint space}: in this other modality not the all joints are considered. Some joint have been blocked: you can move only the $ 1 $°, $ 2 $° and $ 6 $° joint. It's also possible to change the orientation of the EE using angular velocity.
\end{itemize}
...
Using the pad, it's possible to take advantage of the internal vibration. Therefore, when an external force greater than a preset value is detected, the pad start vibrating. Based on the control mode that the user is using (impedance or position control), the activation force is different.
