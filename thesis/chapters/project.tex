...
As described in the next sections, in every phase a way to save waypoints and an action on the gripper was implemented. After that the action was captured by the program and saved in a \texttt{.csv} file. With a dedicated program and that file, all the actions saved into the file can be replicated by the robot. Then the description of the two developed modalities.
	...
	Remote control or \textit{teleoperation} indicates operation of a system or robot at distance. In our case, teleoperation is intended as control the \kuka{} with a \textbf{PlayStation 4 pad} and it was developed in two ways:
