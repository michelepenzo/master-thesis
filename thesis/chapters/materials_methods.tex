...
The values read from the analogs of the pad, now called $\{ \alpha \}$, are between $ -1 $ and $ 1 $. The following formulas permits to transform the values in the correct form for the \kuka{}:
\begin{itemize}
	\item $ \alpha*1000 $: for linear velocity,
	\item $ \alpha*5 $: for angular velocity.
\end{itemize}

The same formula for linear velocity was applied to move the joints. In figure \ref{pad} we can see the structure of the pad and how it was used for control the robot. In both cases, the buttons used are the same:
