...
\end{itemize}
This phase of the test was made by an inexperienced user who had to do the same task for $ N $ times. The value of $ N $ that was chosen was a relatively high value that will be reduced in the final experiment. The task consist to move four Lego blocks from a pick position to a place position. This task was repeated for both the modalities: \kt{} and \te{}. % TODO foto del setup 
...
As represented in figure \ref{fig:diff_kt} we can say that the time necessary to complete the task was similar between the two users, but in the figure \ref{fig:kt_user} the first trial wasn't completed due a mistake by the user. In this case wasn't made to repeat the experiment because this is test and all the possible problems had to be understood. We can also say that with more attempts the time for complete the task is always minor.
...
% cosa abbiamo notato in questi valori? che i primi tentativi falliscono in quanto non si conosce molto bene l'mbiente
% abbiamo capito cosa possiamo misuare e cosa possiamo far svolgere come task alle persone
