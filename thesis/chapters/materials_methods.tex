...
The values read from the analogs of the pad are between $ -1 $ and $ 1 $. These values, that we called $\{ \alpha \}$ for simplicity, allow us to define the following formulas for controlling the \kuka{}:
\begin{itemize}
	\item $ \alpha*1000 $ : for linear velocity,
	\item $ \alpha*5 $ : for angular velocity.
\end{itemize}

The same formula used for linear velocity for the \textit{full task space} mode was applied to move joints in the \textit{reduced joint space} mode. In figure \ref{pad} we can see the structure of the pad and how it was used for control the robot. In both cases, the buttons used are the same:
